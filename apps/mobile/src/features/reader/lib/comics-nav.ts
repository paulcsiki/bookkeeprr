 ...
 * safe to unit-test in a node environment. The component wires these to touch
 * coordinates and gesture events; the math lives here so it stays testable.
 *
 * Ported verbatim from the web reader (`apps/web/src/components/reader/lib/
 * comics-nav.ts`) to keep web/mobile parity; the web unit test vectors are
 * mirrored under `tests/unit/reader/comics-nav.test.ts`.
 */
