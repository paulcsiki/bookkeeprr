...
import { GoogleBooksPutBody } from '@/server/openapi/schemas/settings';
...
export async function PUT(req: Request): Promise<Response> {
  ...
  const parsed = GoogleBooksPutBody.safeParse(body);
  if (!parsed.success) return NextResponse.json({ error: parsed.error.message }, { status: 400 });

  const before = await googleBooksApiKeySetting.get();
  // Empty input means "keep the stored key" — only persist a real new value.
  if (parsed.data.apiKey === '' && before.length > 0) {
    return NextResponse.json({ ok: true });
  }
  await googleBooksApiKeySetting.set(parsed.data.apiKey);
  await recordAuditEvent({
    actor: { kind: 'user', userId: ctx.user.id, username: ctx.user.username },
    action: 'settings.update',
    target: { kind: 'settings', id: 'googlebooks' },
    metadata: {
