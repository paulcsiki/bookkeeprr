...
import { MatcherAdultFilterPatchBody } from '@/server/openapi/schemas/settings';
import { requireAdmin } from '@/server/auth/require-admin';
import { adultFilterSetting, type AdultFilter } from '@/server/db/settings/matcher';
...
export async function PATCH(req: Request): Promise<NextResponse> {
  ...
  const parsed = MatcherAdultFilterPatchBody.safeParse(body);
  if (!parsed.success) {
  ...
  const current = await adultFilterSetting.get();
  const merged: AdultFilter = { ...current, ...parsed.data };
  await adultFilterSetting.set(merged);
  ...
  await recordAuditEvent({
    ...
    target: { kind: 'settings', id: 'matcher-adult-filter' },
