...
import {
  childrenOf, crumbChain, displayPath, flatModeActive, pickerOptions,
  descendantGroupIds, seriesUnderGroup,
  type GroupNode,
...
describe('library groups lib', () => {
  ...
  it('flatModeActive: any non-default facet or search', () => {
    const def = { type: 'all', read: 'all', health: 'all', mon: 'all', q: '' };
    expect(flatModeActive(def)).toBe(false);
    expect(flatModeActive({ ...def, type: 'manga' })).toBe(true);
    expect(flatModeActive({ ...def, q: 'vin' })).toBe(true);
  });
